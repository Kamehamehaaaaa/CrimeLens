...
from Deliverable3.FinalGenerator import (
    CrimeGraphBuilder, 
    CrimeGNN, 
    HypothesisGenerator,
...
class CrimeSentenceParser:
    """Parse crime description sentences into structured data."""
    
    def __init__(self, graph_builder: CrimeGraphBuilder):
        self.graph_builder = graph_builder
        
        # Get known entities from the trained model
        self.known_actions = set(graph_builder.action_encoder.classes_)
        self.known_objects = set(graph_builder.object_encoder.classes_)
        self.known_locations = set(graph_builder.location_encoder.classes_)
    ...
    def parse(self, sentence: str) -> Optional[pd.DataFrame]:
        """
        Parse a crime description sentence into structured DataFrame.
        
        Example input: "John stabbed Maria with a knife at the apartment"
        """
        # Extract entities
        ...
        with st.spinner("Loading model... This may take a minute."):
            try:
                ...
                if os.path.exists("notebooks/results/final_model.pt"):
                    ...
                    df = generate_advanced_data(n_scenes=1500, seed=41)
            ...
            if st.button("🎯 Predict Crime Type", key="predict_btn"):
                with st.spinner("Analyzing..."):
                    ...
                    pred_df = pd.DataFrame([
                        {'Crime Type': k, 'Probability': f"{v*100:.2f}%"} 
